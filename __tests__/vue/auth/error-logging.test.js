...
// by breadcrumb and session-replay telemetry, so the bearer credential must not appear in it — and it is in
// reach twice over: authData.token, and the Authorization header addBearerHeader put on the request.
...
describe("autoLogoutOnFailedRequest logging", () => {
    test("neither the authData token nor the Authorization header reaches the console", async () => {
