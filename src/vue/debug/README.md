...
`regira/vue/debug` — a `<Debug>` display component for dumping values on screen, plus a Vue
...
`configureGlobals({ registerComponentsGlobally: true })` (from `regira/vue/ioc`) set before
