...
A front-end app is **assembled** from several `regira` packages. `vue/entities` is the CRUD
...
| ------------------------------ | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| **vue/entities**               | `regira/vue/entities`                                                                    | **This module.** The CRUD engine: `EntityBase`, `EntityServiceBase`/`JSONService`, `IConfig`, `createStore` (pooling), the overview/details/form/filter composables, navigation, preloading, tree.                                                                                                                                     | ✓ core                                                                                    |
| **vue/app**                    | [`regira/vue/app`](../../app/ai/app.instructions.md)                                     | App lifecycle + culture: `AppStatus` (Init→Mounting→Ready), `$setAppStatus`, `$culture`/`$setCulture`, `whenAppReady()`, the loading gate. The app's heartbeat.                                                                                                                                                                        | ✓ core                                                                                    |
| **vue/ioc**                    | [`regira/vue/ioc`](../../ioc/ai/ioc.instructions.md)                                     | Service container: register the shared `axios` + `PoolCache` here; every entity service is resolved from it by `Entity.name` via `get()`.                                                                                                                                                                                              | ✓ core                                                                                    |
| **vue/http**                   | [`regira/vue/http`](../../http/ai/http.instructions.md)                                  | The single shared axios: `initAxios({ api, includeCredentials })` (sets `baseURL`, credentials, file upload/download helpers).                                                                                                                                                                                                         | ✓ core                                                                                    |
| **vue/ui**                     | [`regira/vue/ui`](../../ui/ai/ui.instructions.md)                                        | Component + feedback kit (Bootstrap 5): `feedbackPlugin`, `loadingPlugin`/`LoadingContainer`, `iconPlugin`, `screenPlugin`, `DefaultModal`, inputs (`DateInput`, `NullableCheckBox`…), `Autocomplete`, paging, tabs — components are imported locally by default (opt-in app-wide registration via `configureGlobals` from `vue/ioc`). | ✓ core¹                                                                                   |
| **vue/auth**                   | [`regira/vue/auth`](../../auth/ai/auth.instructions.md)                                  | Login + bearer token layered on the shared axios: `authPlugin`, `LocalStorageTokenManager`, `useAuthStore`, login UI, route guard.                                                                                                                                                                                                     | ○ optional (see [Running without auth](entities.setup.md#running-without-authentication)) |
| **vue/lang**                   | [`regira/vue/lang`](../../lang/ai/lang.instructions.md)                                  | i18n: `langPlugin`, `useLang`, `$t`/`$tm`, key-first translations.                                                                                                                                                                                                                                                                     | ○ optional                                                                                |
| **vue/directives**             | [`regira/vue/directives`](../../directives/ai/directives.instructions.md)                | Global directives installed as plugins: `focus`, `grow` (textarea autosize), `clickOutside`.                                                                                                                                                                                                                                           | ○ optional                                                                                |
| **vue/online**                 | [`regira/vue/online`](../../online/ai/online.instructions.md)                            | Connectivity: `isOnlinePlugin` + `$isOnline`, drives an offline banner.                                                                                                                                                                                                                                                                | ○ optional                                                                                |
| **vue/formatters**             | [`regira/vue/formatters`](../../formatters/ai/formatters.instructions.md)                | Date/number formatting (`formatDateTime`, …) — used for display and config cache-busting.                                                                                                                                                                                                                                              | ○ optional                                                                                |
| **vue/debug**                  | [`regira/vue/debug`](../../debug/ai/debug.instructions.md)                               | Dev-only `debugPlugin` + `$isDebug`/`$setDebug`.                                                                                                                                                                                                                                                                                       | ○ optional                                                                                |
| **extensions/date-extensions** | [`regira/extensions/date-extensions`](../../../extensions/ai/extensions.instructions.md) | `dateSerializer.use()` once at startup — serialize `Date`s to JSON without a timezone shift. Lives under `extensions/`, **not** `vue/`.                                                                                                                                                                                                | ○ recommended                                                                             |
| **utilities**                  | [`regira/utilities`](../../../utilities/ai/utilities.instructions.md)                    | Pure helpers (`string-utility`, `array-utility`, `file-utility`, …).                                                                                                                                                                                                                                                                   | ○ as needed                                                                               |

> **Not in the common stack:** `regira/treelist` (`TreeList` / `IFindParents`) is a direct
...
- `processItem` converts the string fields **`created`** and **`lastModified`** into `Date` instances
  ...
  `dateInputString(date?)` formatter (`yyyy-MM-dd`) from `regira/vue/formatters`.
...
`node node_modules/regira/_template/scaffold.mjs <Entity>` (see [slice template](entities.template.md)).
...
one command: `node node_modules/regira/_template/scaffold.mjs --shell` (`--no-auth` for a no-auth app).
...
`node node_modules/regira/_template/scaffold.mjs <Entity>`, then fill the `(c)` files in this order.
...
> `translate` comes from `const { translate } = useLang()` in **`regira/vue/lang`** — the first
...
| `Maximum recursive updates exceeded`, blamed on a library component | A `toEntity` that mutates unconditionally (`item.created = new Date(item.created)`); it runs inside a computed (`FormModalButton.modalTitle`), so it retriggers itself | Make `toEntity` **idempotent** — return the instance untouched when it already is one, and only convert a field that isn't converted yet                                                                                                                                                                                                                                                                              |
...
| `TS2554: Expected 1 arguments, but got 2` on template lines the scaffold wrote (`@save="$emit('save', $event)"`) | A custom event was added by **intersecting** the emits type: `defineEmits<Emits & { (e: "reload"): void }>()`. TypeScript resolves the template's `$emit` to that last, narrowest call signature, so every two-argument emit in the file fails | Extend by listing the call signatures instead — `interface Emits extends /* @vue-ignore */ OverviewEmits<Entity> { (e: "reload"): void }`, then the bare `defineEmits<Emits>()`. This is the shape `overview/List.vue` already uses |

> **Dormant code — do not use:**
>
> - `regira/entities` (the `src/entities/*` folder) is fully commented-out; its `package.json`
>   `./entities` export resolves to an empty module. Use `regira/vue/entities`.
