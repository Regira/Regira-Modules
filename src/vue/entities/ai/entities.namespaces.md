...
| Form                   | `useForm`, `useModal`, `FormProps`, `FormEmits`, `FormIn`, `FormOut`, `FormModalIn`, `FormModalOut`, `FormStates`, `formDefaults`, `useOwnedCollection`, `useOwnedModal`, `useListInput`, `useListItemInput`, `InputSelectorInline` (+ `InputSelectorInlineProps/Emits/Slots`) |
...
| `@regira/modules/vue/formatters` | `formatDateTime(date, mask)`, `formatDate`/`formatShortDate`/`formatNumber`/`formatCurrency`/`formatPercentage` (2nd arg is a **culture**; `formatNumber` prints 2 decimals unless its 3rd arg says otherwise — a count is `formatNumber(n, culture, 0)`), the fixed-mask `formatTime`/`dateInputString`/`dateTimeInputString`, `getInitials`, `shortenString` | rendering dates, numbers and money — do not hand-roll these                                                                                |
