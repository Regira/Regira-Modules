...
or on failure `fail(...)`. It does not re-throw, so `@submit.prevent="handleSubmit"` is safe to bind
directly; branch on `feedback` when you need to know the outcome. The failure mapping is fixed:
