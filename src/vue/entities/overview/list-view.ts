...
export function useListView<T extends IEntity, SO extends ISearchObject = ISearchObject>({
    ...
    // Only the newest list may write — the same reasoning as `useSearchView`'s `searchHandler`: on a hard
    // reload a mount fetch and the `onAuthenticated` re-run overlap, and the one that settles last would
    // otherwise win, up to painting a 401's banner (`fail()` does not auto-hide) over rows already on screen.
    let latestListId = 0
    async function listHandler(): Promise<void> {
        const listId = ++latestListId
        ...
        try {
            ...
            if (listId === latestListId) isLoading.value = false
        }
    }
